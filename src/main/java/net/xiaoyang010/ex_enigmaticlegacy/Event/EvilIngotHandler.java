...
public class EvilIngotHandler {

    private static final ConcurrentHashMap<UUID, Boolean> playerHasevilIngot = new ConcurrentHashMap<>();
    private static int colorTick = 0;

    @SubscribeEvent
    public static void onItemPickup(TickEvent.PlayerTickEvent event) {
        if (event.player.getInventory().contains(new ItemStack(EnigmaticLegacy.evilIngot))) {
            event.player.getPersistentData().putBoolean("hasevilIngot", true);
        }
    }
    ...
    public static class ClientEvents {
        ...
        public static void onTooltipRender(RenderTooltipEvent.GatherComponents event) {
            ...
            if (stack.getItem() == ModBlockss.EVILBLOCK.get().asItem()) {
                ...
                } else if (hasevilIngot && !tooltipComponents.isEmpty()) {
