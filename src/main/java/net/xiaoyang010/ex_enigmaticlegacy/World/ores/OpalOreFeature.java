...
public class OpalOreFeature extends OreFeature {
    ...
    public static Feature<?> feature() {
        ...
        CONFIGURED_FEATURE = FeatureUtils.register("ex_enigmaticlegacy:opal_ore", FEATURE, new OreConfiguration(OpalOreFeatureRuleTest.INSTANCE, ((Block) ModBlockss.OPAL_ORE.get()).defaultBlockState(), 6));
        PLACED_FEATURE = PlacementUtils.register("ex_enigmaticlegacy:opal_ore", CONFIGURED_FEATURE, List.of(CountPlacement.of(2), InSquarePlacement.spread(), HeightRangePlacement.uniform(VerticalAnchor.absolute(-30), VerticalAnchor.absolute(60)), BiomeFilter.biome()));
        return FEATURE;
    }
    ...
    private static class OpalOreFeatureRuleTest extends RuleTest {
        ...
        private static final Codec<OpalOreFeatureRuleTest> CODEC = Codec.unit(() -> {
            return INSTANCE;
        });
        private static final RuleTestType<OpalOreFeatureRuleTest> CUSTOM_MATCH = () -> {
            return CODEC;
        };
        private List<Block> base_blocks = null;

        private OpalOreFeatureRuleTest() {
        }

        @SubscribeEvent
        public static void init(FMLCommonSetupEvent event) {
            Registry.register(Registry.RULE_TEST, new ResourceLocation("ex_enigmaticlegacy:opal_ore_match"), CUSTOM_MATCH);
        }
