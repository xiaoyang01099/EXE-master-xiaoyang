...
public class JadeOreFeature extends OreFeature {
    public static JadeOreFeature FEATURE = null;
    ...
    private static class JadeOreFeatureRuleTest extends RuleTest {
        ...
        private static final Codec<JadeOreFeatureRuleTest> CODEC = Codec.unit(() -> {
            return INSTANCE;
        });
        private static final RuleTestType<JadeOreFeatureRuleTest> CUSTOM_MATCH = () -> {
            return CODEC;
        };
        private List<Block> base_blocks = null;

        private JadeOreFeatureRuleTest() {
        }

        @SubscribeEvent
        public static void init(FMLCommonSetupEvent event) {
            Registry.register(Registry.RULE_TEST, new ResourceLocation("ex_enigmaticlegacy:jade_ore_match"), CUSTOM_MATCH);
        }
