...
public class RenderTileAdvancedSpreader implements BlockEntityRenderer<TileAdvancedSpreader> {

    public RenderTileAdvancedSpreader(BlockEntityRendererProvider.Context context) {}

    @Override
    public void render(@Nonnull TileAdvancedSpreader spreader, float partialTicks, PoseStack ms, MultiBufferSource buffers, int light, int overlay) {
        ms.pushPose();
        ...
        float g = 1.0F;
        float b = 1.0F;
        if (spreader.getAdvancedVariant() == BlockAdvancedSpreader.VariantN.NATURE) {
            int color = Mth.hsvToRgb((float)((time * 2.0 + (double)(new Random(spreader.getBlockPos().hashCode())).nextInt(10000)) % 360.0) / 360.0F, 0.4F, 0.9F);
    ...
    private BakedModel getCoreModel(TileAdvancedSpreader tile) {
        BakedModel var10000;
        switch (tile.getAdvancedVariant()) {
            case NATURE -> var10000 = SpecialMiscellaneousModels.INSTANCE.advancedSpreaderCore;
    ...
    private BakedModel getScaffoldingModel(TileAdvancedSpreader tile) {
        BakedModel var10000 = switch (tile.getAdvancedVariant()) {
            case NATURE -> MiscellaneousModels.INSTANCE.elvenSpreaderScaffolding;
            default -> throw new IncompatibleClassChangeError();
        };
