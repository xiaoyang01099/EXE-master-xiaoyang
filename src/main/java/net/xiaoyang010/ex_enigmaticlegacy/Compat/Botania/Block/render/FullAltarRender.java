...
public class FullAltarRender implements BlockEntityRenderer<FullAltarTile> {
    private final BlockRenderDispatcher blockRenderDispatcher;

    public FullAltarRender(BlockEntityRendererProvider.Context context) {
        this.blockRenderDispatcher = context.getBlockRenderDispatcher();
    }

    @Override
    public void render(@Nonnull FullAltarTile altar, float pticks, PoseStack ms, MultiBufferSource buffers, int light, int overlay) {
        ms.pushPose();
        ...
        if (water) {
            ...
            if (altar.getLevel() != null) {
                color = BiomeColors.getAverageWaterColor(altar.getLevel(), altar.getBlockPos());
            }
            VertexConsumer buffer = buffers.getBuffer(Sheets.translucentCullBlockSheet());
            renderIcon(ms, buffer, sprite, color, alpha, overlay, light);
            ms.popPose();
        }
        ms.popPose();
    }
