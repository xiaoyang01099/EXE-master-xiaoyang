...
public class InfinityGaiaSpreaderRenderer implements BlockEntityRenderer<InfinityGaiaSpreaderTile> {

    public InfinityGaiaSpreaderRenderer(BlockEntityRendererProvider.Context context) {}

    @Override
    public void render(@Nonnull InfinityGaiaSpreaderTile spreader, float partialTicks, PoseStack ms, MultiBufferSource buffers, int light, int overlay) {
        ms.pushPose();
        ...
        float g = 1.0F;
        float b = 1.0F;
        if (spreader.getVariant() == VariantE.GAIA) {
        ...
        if (!stack.isEmpty()) {
            ...
            Minecraft.getInstance().getItemRenderer().renderStatic(stack, TransformType.NONE, light, overlay, ms, buffers, 0);
            ms.popPose();
        }
    ...
    private BakedModel getCoreModel(InfinityGaiaSpreaderTile tile) {
        BakedModel var10000;
