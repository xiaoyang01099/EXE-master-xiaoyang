        ...
        for (int i = 0; i < sprites.length; i++) {
            ...
            for (BlockElement element : unbaked) {
                for (Map.Entry<Direction, BlockElementFace> entry : element.faces.entrySet()) {
                    quads.add(FACE_BAKERY.bakeQuad(element.from, element.to, entry.getValue(), sprite, entry.getKey(), SimpleModelState.IDENTITY, element.rotation, element.shade, new ResourceLocation(ExEnigmaticlegacyMod.MODID, "dynamic")));
                }
            }
        }
        return quads;
