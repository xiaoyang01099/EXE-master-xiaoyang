...
public class RenderBabylonWeaponSS extends EntityRenderer<EntityBabylonWeaponSS> {

    public RenderBabylonWeaponSS(EntityRendererProvider.Context ctx) {
        super(ctx);
    }

    @Override
    public void render(@Nonnull EntityBabylonWeaponSS weapon, float yaw, float partialTicks, PoseStack ms, MultiBufferSource buffers, int light) {
        ms.pushPose();
        ...
        ms.pushPose();
        ms.translate(-0.75, 0, 1);
        float s = 1.5F;
        ms.scale(s, s, s);
        ms.mulPose(Vector3f.YP.rotationDegrees(90F));
        ms.mulPose(Vector3f.ZP.rotationDegrees(-45F));
        ...
        ms.translate(0F, -0.3F + rand.nextFloat() * 0.1F, 0F);
    ...
    @Override
    public ResourceLocation getTextureLocation(@Nonnull EntityBabylonWeaponSS entity) {
